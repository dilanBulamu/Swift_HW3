...
protocol StackDisplayLogic: class {
    func displayFetchFromLocalDataStore(with viewModel: StackModels.FetchFromLocalDataStore.ViewModel)
    func displayFetchFromRemoteDataStore(with viewModel: StackModels.FetchFromRemoteDataStore.ViewModel)
    func displayTrackAnalytics(with viewModel: StackModels.TrackAnalytics.ViewModel)
    func displayPerformStack(with viewModel: StackModels.PerformStack.ViewModel)
}

class StackViewController: UIViewController, StackDisplayLogic {

    // MARK: - Properties

    typealias Models = StackModels
    var router: (NSObjectProtocol & StackRoutingLogic & StackDataPassing)?
    var interactor: StackBusinessLogic?
    ...
    func trackAnalytics(event: StackModels.AnalyticsEvents) {
    ...
    func performStack(_ sender: Any) {
        let request = Models.PerformStack.Request(exampleVariable: exampleLocalLabel.text)
        interactor?.performStack(with: request)
    }
